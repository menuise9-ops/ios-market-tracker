...
def render(config):
    ...
    for i, v in enumerate(data["vendors"]):
        with st.container(border=True):
            ...
            c4.markdown(f"<div style='text-align:right'><b>{esc_md(fmt_money(v['total_volume']))}</b><br>"
                        f"<span style='font-size:11px;color:#94a3b8'>avg {esc_md(fmt_money(v['avg_deal_size']))}</span></div>", unsafe_allow_html=True)
